...
export class AuctionParachain implements Entity {
    ...
    static async getByAuctionId(auctionId: string): Promise<AuctionParachain[] | undefined>{
      
      const records = await store.getByField('AuctionParachain', 'auctionId', auctionId);
      return records.map(record => AuctionParachain.create(record));
      
    }

    static async getByParachainId(parachainId: string): Promise<AuctionParachain[] | undefined>{
      
      const records = await store.getByField('AuctionParachain', 'parachainId', parachainId);
      return records.map(record => AuctionParachain.create(record));
