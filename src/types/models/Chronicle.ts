...
export class Chronicle implements Entity {
    ...
    static async getByCurAuctionId(curAuctionId: string): Promise<Chronicle[] | undefined>{
      
      const records = await store.getByField('Chronicle', 'curAuctionId', curAuctionId);
      return records.map(record => Chronicle.create(record));
